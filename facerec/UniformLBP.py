    ...
    def run(self, img, debug=False):
        ...
        for x in range(0, len(img)):
            for y in range(0, len(img[0])):
                ...
                # Add the pattern to the uniform bins if not already
                if pattern not in uniform_bin:
                    uniform_bin.append(pattern)
                    transformed_img.itemset((x, y), pattern)
                    uniform_values.append(pattern)
        ...
        return np.histogram(uniform_values, 59)
