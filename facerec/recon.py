...
def write_pcd(fn, verts, colors, disp, face_coords, write=False, mutate=False):
    ...
    # rec_verts = np.random.normal(rec_verts, 0.01)
...
def main(leftpath_, rightpath_, filename):
    ...
    if leftpath_ == 'enrol':
        subjects.append(rightpath_)
        with open('subjects', 'wb') as fp:
            pickle.dump(subjects, fp)


    for idx, subject in enumerate(subjects):
        leftpath = '/home/arthur/stereo_demo/%s/right.png' % subject
        rightpath = '/home/arthur/stereo_demo/%s/left.png' % subject
        std, rec_data, face_coords = generate_encoding(leftpath, rightpath, detector, stereo, 'ignore', mutate=False)
        faces.append(rec_data)
        labels.append(idx)

    mutate_subjects = ['Alfonso_Soriano', 'Amanda_Beard', 'Adam_Kennedy']
    ...
    for idx, score in enumerate(y_score):
        ...
        if score > (len(mutate_subjects) + 0.2):
            detected = idx
    ...
    if detected is not False and photo is False:
        color = (0, 255, 0)
