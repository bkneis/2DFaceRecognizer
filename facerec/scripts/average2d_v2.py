...
def traverse(root_dir):
    train_dir = '/home/arthur/ps_train'
    test_dir_directional = '/home/arthur/ps_test_directional'
    test_dir_intense = '/home/arthur/ps_test_intense'
    test_dir_average = '/home/arthur/ps_test_average'
    ...
    for idx, subject_dir in enumerate(subject_dirs):
        ...
        test_directional_subject_path = os.path.join(test_dir_directional, subject_dir)
        if not os.path.exists(test_directional_subject_path):
            os.makedirs(test_directional_subject_path)

        test_intense_subject_path = os.path.join(test_dir_intense, subject_dir)
        if not os.path.exists(test_intense_subject_path):
            os.makedirs(test_intense_subject_path)

        test_average_subject_path = os.path.join(test_dir_average, subject_dir)
        if not os.path.exists(test_average_subject_path):
            os.makedirs(test_average_subject_path)
        ...
        for session_id, sessionDir in enumerate(train_dirs):
            ...
            if img0 is not None:
                ...
                cv2.imwrite(train_subject_path + '/image-%s.png' % session_id, average)
        ...
        for session_id, sessionDir in enumerate(test_dirs):
            ...
            if img0 is not None:
                ...
                cv2.imwrite(test_average_subject_path + '/image-%s.png' % session_id, average)
                cv2.imwrite(test_intense_subject_path + '/image-%s.png' % session_id, left)
                cv2.imwrite(test_intense_subject_path + '/image-%s.png' % session_id, img0)
